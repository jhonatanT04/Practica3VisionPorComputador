...
vector<vector<Point>> Ania::bordesPuntos(Mat bin)
{
...
vector<Vec4i> Ania::bordesPuntosJerarquia(Mat bin)
{
    vector<vector<Point>> puntos;
    vector<Vec4i> jerarquia;
...
Point Ania::centroideIMG(Mat bordesCanny)
{
...
Mat Ania::coordennadasCompleja(vector<Point> &contorno, Point2f centroide)
{
...
Mat Ania::calculoDFT(Mat coordenadasComplejas)
{
...
float Ania::distaciaEuclider(vector<float> desc1, vector<float> desc2)
{
...
vector<float> Ania::normalizarDescriptores(Mat dftResult)
{
...
float Ania::distaciaFiguras(vector<vector<float>> distancias, vector<float> img)
{
...
String Ania::clasificacionFigura(Mat img)
{
